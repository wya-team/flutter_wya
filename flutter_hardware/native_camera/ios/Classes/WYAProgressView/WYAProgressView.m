...
- (void)layoutSubviews
{
    ...
    if (self.style == WYAProgressViewStyleStraight) {
        ...
        CGFloat bgImageView_Width  = self.frame.size.width * self.progress;
        CGFloat bgImageView_Height = self.frame.size.height;
        ...
        CGFloat bgImageView_Width  = self.frame.size.width - self.borderWidth * 2;
        CGFloat bgImageView_Height = self.frame.size.width - self.borderWidth * 2;
        self.bgImageView.frame =
        CGRectMake(bgImageView_X, bgImageView_Y, bgImageView_Width, bgImageView_Height);

        self.bgImageView.layer.cornerRadius = (self.frame.size.width - self.borderWidth * 2) / 2;
    }
}
...
- (CAShapeLayer *)backLayer
{
    if (!_backLayer) {
        ...
        _backLayer.frame       = CGRectMake(0, 0, self.frame.size.width, self.frame.size.width);
...
- (void)setBorderWidth:(CGFloat)borderWidth
{
    if (_borderWidth != borderWidth) {
        ...
        _radius = self.frame.size.width / 2.0 - _borderWidth / 2.0;
...
- (UIBezierPath *)getNewBezierPath
{
    if (self.style == WYAProgressViewStyleStraight) {
        ...
        return [UIBezierPath
        bezierPathWithArcCenter:CGPointMake(self.frame.size.width / 2.0, self.frame.size.width / 2.0)
                         radius:_radius
