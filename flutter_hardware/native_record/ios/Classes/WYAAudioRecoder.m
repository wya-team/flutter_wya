...
@property (nonatomic, assign) NSTimeInterval currentTimeInt;
@end
...
//    }
    ...
    NSError * activeError;
    [self.session setActive:YES error:&activeError];
    if (activeError != nil) {
        return NO;
    }
...
- (void)wya_pausePlayAudio
{
    ...
    self.currentTimeInt = self.audioPlayer.currentTime;
}

- (void)wya_resumePlayAudio
{
    [self.audioPlayer playAtTime:self.currentTimeInt];
}
